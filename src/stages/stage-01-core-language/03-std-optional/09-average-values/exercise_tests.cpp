...
int main() {
    using stage_01::std_optional::average_values_exercise::average_values;
    try {
        EXPECT_OPTIONAL_NEAR(average_values({1.0, 2.0, 3.0}), std::optional<double>{2.0});
        EXPECT_OPTIONAL_NEAR(average_values({2.5, 3.5}), std::optional<double>{3.0});
        EXPECT_OPTIONAL_NEAR(average_values({}), std::optional<double>{});
