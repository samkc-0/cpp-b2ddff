...
int main() {
    using stage_01::std_optional::find_first_even_exercise::find_first_even;
    try {
        EXPECT_EQUAL(find_first_even({1, 3, 4, 6}), std::optional<int>{4});
        EXPECT_EQUAL(find_first_even({2, 4, 6}), std::optional<int>{2});
        EXPECT_EQUAL(find_first_even({1, 3, 5}), std::optional<int>{});
        EXPECT_EQUAL(find_first_even({}), std::optional<int>{});
