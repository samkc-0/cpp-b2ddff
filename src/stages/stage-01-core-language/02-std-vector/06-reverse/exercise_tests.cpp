...
int main() {
    using stage_01::std_vector::reverse_exercise::reverse_vector;
    try {
        EXPECT_EQUAL(reverse_vector({1, 2, 3}), std::vector<int>({3, 2, 1}));
        EXPECT_EQUAL(reverse_vector({7}), std::vector<int>({7}));
        EXPECT_EQUAL(reverse_vector({}), std::vector<int>{});
