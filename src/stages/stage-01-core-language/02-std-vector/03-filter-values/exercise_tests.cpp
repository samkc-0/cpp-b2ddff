...
int main() {
    using stage_01::std_vector::filter_values_exercise::filter_values;
    try {
        EXPECT_EQUAL(filter_values({1, 5, 2, 8, 3}, 4), std::vector<int>({5, 8}));
        EXPECT_EQUAL(filter_values({4, 4, 3}, 4), std::vector<int>({4, 4}));
        EXPECT_EQUAL(filter_values({}, 10), std::vector<int>{});
